...
public class Hyperzoom : HyperzoomManagement
{
	...
	void ZoomCleanup(bool useless = false)
	{
		...
		if (zoomTargetPct < zoomFadeMargin)
		{
			...
			else if (fungusSceneManager != null && !fungusSceneManager.RequestedSceneIsValid())
			{
			...
			else if (fungusSceneManager != null && zoomTargetPct <= zoomPointOfNoReturnLow)
			{
				snapTarget = 0.0f;
			}
		...
		if (zoomTargetPct > 1.0f - zoomFadeMargin)
		{
			...
			if (fungusSceneManager != null && !fungusSceneManager.RequestedSceneIsValid())
			{
				// abort force zoom out. Snap back to Margin edge
				snapTarget = 1.0f - zoomFadeMargin;
			}
			// if we're beyond the point of no return and the manager is present
			else if (fungusSceneManager != null && zoomTargetPct >= zoomPointOfNoReturnHigh)
			{
				// force zoom out
				snapTarget = 1.0f;
			}
